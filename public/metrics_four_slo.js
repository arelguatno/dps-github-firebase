...
document.getElementById("exportBtn").addEventListener("click", myExportCSV);
...
var html_link = "";
db.collection("sample_links").where("link_number", "==", 4)
    .get()
    .then((querySnapshot) => {
...
const letsGo = async () => {
    ...
    logReport("Issue#,Date Created,State,--,SLO - 8Hrs Triage,Event,Date,--,SLO - First Repro Attempt,Event,Date,--,SLO - Second Repro Attempt,Event,Date");
    logReport("\n");
    ...
    for (var i = 0, length = keys.length; i < length; i++) {
        ...
        if (myJson[i].pull_request == undefined && (date_created.getTime() >= date_queryy.getTime())) { // only those Github issues
            ...
            var slo_triage_first_hrs = 'N/A';
            var slo_triage_first_hrs_event = '';
            var slo_triage_first_hrs_date = '';

            var slo_triage_second_hrs = 'N/A';
            var slo_triage_second_hrs_event = '';
            var slo_triage_second_hrs_date = '';

            var reporter_id = '';
            var data_fullfilled = false;
            var engineer_handled = false;
            ...
            for (var y = 0, lengths = myTimelineKeys.length; y < lengths; y++) {
                ...
                // (3) Support tagged to the correct API label
                for (var x = 0, length2 = myTimelineKeys.length; x < length2 && !data_fullfilled; x++) {
                ...
                // (4) Support removed needs triage or new label
                for (var x = 0, length2 = myTimelineKeys.length; x < length2 && !data_fullfilled; x++) {
                ...
                // (5) If bot labeled the issue correctly and not change form the support
                for (var x = 0, length2 = myTimelineKeys.length; x < length2 && !data_fullfilled; x++) {
                    if (myTimeline[x].actor.id == google_oos_bot_uid // bot labeled
                        && myTimeline[x].event == "labeled"    // should be labeled
                        && myTimeline[x].actor.id != reporter_id  // should not be same reporter
                        && myTimeline[x].label.name.indexOf("api") >= 0) { // should have api label

                        slo_triage_8_hrs = checkEightHoursSLO(date_logged, myTimeline[x].created_at);
                        slo_triage_8_hrs_event = 'Bot labeled API. No change from the support.';
                        slo_triage_8_hrs_date = formatDate(myTimeline[x].created_at);
                        data_fullfilled = true;
                        break;
                ...
                // (6) If cannot repro, asked for the additional info and labelled case as need_info
                for (var x = 0, length2 = myTimelineKeys.length; x < length2 && !data_fullfilled; x++) {
                ...
                // (7) Feature requests are identified and labelled as appropriate
                for (var x = 0, length2 = myTimelineKeys.length; x < length2 && !data_fullfilled; x++) {
                    if (myTimeline[x].actor.id != google_oos_bot_uid // not bot
                        && myTimeline[x].event == "labeled"    // should be labeled
                        && myTimeline[x].actor.id != reporter_id  // should not be same reporter
                ...
                // (8) If known bugs, tagged accordingly
                for (var x = 0, length2 = myTimelineKeys.length; x < length2 && !data_fullfilled; x++) {
                    if (myTimeline[x].actor.id != google_oos_bot_uid // not bot
                        && myTimeline[x].event == "labeled"    // should be labeled
                        && myTimeline[x].actor.id != reporter_id  // should not be same reporter
                ...
                // (10) Support commented and Closed the issue
                ...
                // (11) Support provided first response
                for (var x = 0, length2 = myTimelineKeys.length; x < length2 && !data_fullfilled; x++) {
                ...
                // (12) Support removed needs triage or new label
                for (var x = 0, length2 = myTimelineKeys.length; x < length2 && !data_fullfilled; x++) {
                ...
                if (slo_triage_8_hrs == 'No') {
                    slo_triage_8_hrs = 'Out of condition'
                }



                const response = await getFirstReproDate(getRepoName(repo_name), issue_number.toString());
                if (response !== null) {

                    const workLogs = response.worklogs;
                    for (var key in workLogs) {
                        var data = response.worklogs[key].data;
                        var getFirstRepro = data.indexOf("to <b>First repro in progress</b>");
                        var secondRepro = data.indexOf("to <b>Second repro in progress</b>");
                        // First repro attempt
                        if (getFirstRepro >= 0) {
                            slo_triage_first_hrs = "Pass";
                            slo_triage_first_hrs_event = "From Extension";
                            slo_triage_first_hrs_date = formatDate(workLogs[key].timeStamp);
                        }
                        // Second repro attempt
                        if (secondRepro >= 0) {
                            slo_triage_second_hrs = "Pass";
                            slo_triage_second_hrs_event = "From Extension";
                            slo_triage_second_hrs_date = formatDate(workLogs[key].timeStamp);
                        }
                    }
                }
                ...
                valueToPush.push("--");
                valueToPush.push(slo_triage_first_hrs);
                valueToPush.push(slo_triage_first_hrs_event);
                valueToPush.push(slo_triage_first_hrs_date);
                valueToPush.push("--");
                valueToPush.push(slo_triage_second_hrs);
                valueToPush.push(slo_triage_second_hrs_event);
                valueToPush.push(slo_triage_second_hrs_date);
                arrayValue.push(valueToPush);

                logReport(issue_number + ','
                    + formatDate(date_created.getTime()) + ','
                    + capitalizeFirstLetter(myJson[i].state) + ','
                    + "--" + ','
                    + slo_triage_8_hrs + ','
                    + slo_triage_8_hrs_event + ','
                    + slo_triage_8_hrs_date + ','
                    + "--" + ','
                    + slo_triage_first_hrs + ','
                    + slo_triage_first_hrs_event + ','
                    + slo_triage_first_hrs_date + ','
                    + "--" + ','
                    + slo_triage_second_hrs + ','
                    + slo_triage_second_hrs_event + ','
                    + slo_triage_second_hrs_date + ',');
                break;
            }
            logReport("\n");
            valueToPush = [];
...
function downloadFile(urlData) {
    ...
    if (window.navigator.msSaveOrOpenBlob)  // IE hack; see http://msdn.microsoft.com/en-us/library/ie/hh779016.aspx
        ...
        a.download = "" + repo_name + " " + queryDate + "slo.csv";
...
// Return the SLO expiration date
...
// Return the SLO expiration date
function getFirstReproSLO(date1) {
    var date_to_date = new Date(date1);
    const slo_hrs = 8;
    const shift_start = "17"; //5PM
    const shift_end = "03";  //3AM
...
// var slo = new Date(getFirstReproSLO("2021-05-13 8:00 PM"));
// console.log(slo);


// "First repro"
// "Second repro"
// db.collection("js").doc("4879")
//     .get().then(function (doc) {
//         if (doc.exists) {
//             const workLogs = doc.data().worklogs;
//             for (var key in workLogs) {
//                 var data = workLogs[key].data;
//                 var getFirstRepro = data.indexOf("First");
//                 if (getFirstRepro >= 0) {
//                     console.log(workLogs[key].timeStamp);
//                 }
//             }
//         } else {
//             console.log(null);
...
//     const response = await getFirstReproDate("js", "4882");
...
//             var getFirstRepro = data.indexOf("to <b>First repro in progress</b>");
//             if (getFirstRepro >= 0) {
//                 console.log(formatDate(workLogs[key].timeStamp));
//             }
//         }
//     }
